...
namespace mnist
{
    class Program
    {
        ...
        static readonly string _datapath = Path.Combine(Environment.CurrentDirectory, "Data", "optdigits-train.csv");
        static readonly string _testdatapath = Path.Combine(Environment.CurrentDirectory, "Data", "optdigits-val.csv");
        static readonly string _modelpath = Path.Combine(Environment.CurrentDirectory, "MLModels", "Model.zip");
        ...
        public static void Train(MLContext env)
        {
            try
            {
                ...
                var reader = env.Data.CreateTextReader(new TextLoader.Arguments { Column = new[] { new TextLoader.Column("PixelValues", DataKind.R4, 0, 63), new TextLoader.Column("Number", DataKind.R4, 64) }, Separator = ",", HasHeader = false });
        ...
        private static void TestSomePredictions(MLContext mlContext)
        {
            ...
            var resultprediction1 = predEngine.Predict(data1);
            ...
            var resultprediction2 = predEngine.Predict(data2);
            ...
            var resultprediction3 = predEngine.Predict(data3);

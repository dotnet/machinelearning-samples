...
## Problem
This problem is to classify whether a URL is malicious or not. To solve this problem, we will build an ML model by using  one of the Binary Classification algorithms i.e 

ML.Net can work for large datasets upto 1TB.  
...
Here we are downloading the dataset as it is very large as we don't want to store large files in GITHUB. The dataset consists of 120 days of data. If you open the fodler where dataset is downloaded it contains number of files from day 1 to day 120. Remove the 'FeatureTypes' file from the dataset.
...
* Define the data's schema maped to the datasets to load (training data and `HeartTest.csv`) with a TextLoader.

* You can load more than one file using Texloader by specifying dataPath using [MultiFilesource](https://docs.microsoft.com/en-us/dotnet/api/microsoft.ml.data.multifilesource?view=ml-dotnet). you need to specify 'directoryname/*' to get all files in the directory.

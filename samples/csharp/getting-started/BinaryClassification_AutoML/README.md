...
## Problem
This problem is to predict if a customer's review has a positive or negative sentiment. We will use small wikipedia-detox-datasets (one dataset for training and a second to test model's accuracy). Human judges read every comment in these datasets and assigned each a sentiment label: 
* 0 - nice/positive
* 1 - toxic/negative
